...
public class Parser{
    ...
    public static String[][] parse(String[][] output) {
        ...
        for (int i = 0; i < output.length; i++) {
            for (int j = 0; j < output[i].length; j++) {
                ...
                switch (j) {
                    case 0:
                        assign_workers(shift, startTime, i, j, output, "7-15", Integer.parseInt(output[i][j]));
                        break;
                    case 1:
                        assign_workers(shift, startTime, i, j, output, "15-23", Integer.parseInt(output[i][j]));
                        break;
                    case 2:
                        assign_workers(shift, startTime, i, j, output, "23-7", Integer.parseInt(output[i][j]));
                        break;
        ...
        for (int i = 0; i < week_shift.length; i++) {
            for (int j = 0; j < week_shift[0].length; j++) {
                if (i == 0){
                    ...
                    System.out.println(output[j][i-1]);
                }
            }
        }
        return week_shift;
    ...
    public static boolean suitable_SL(String shift_time, int id, int day, List<Worker> shift){
        ...
        return (workers.get(id).getDay(day) == null // worker has not requested the day off
                ...
                && workers.get(id).getSeniority().equals("SL"); // worker has the seniority "SL" assigned to them

    }

    public static void assign_workers(List<Worker> shift, Long startTime, int i, int j, Object[][] output, String shift_time, int shiftRequiredSize) {
    ...
    public static Worker assign_SL(List<Worker> shift, Long startTime, int i, int j, Object[][] output, String shift_time, int shiftRequiredSize) {
